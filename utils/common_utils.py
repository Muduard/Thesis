...
def get_image(path, imsize=-1, odd=True):
...
def get_noise(input_depth, method, spatial_size, noise_type='u', var=1./10):
    ...
    if method == 'noise':
        ...
        net_input = np_to_torch(meshgrid)
         ...
         shape = [1, input_depth, spatial_size[0], spatial_size[1]]
         net_input = torch.randn(shape) * 50 + 200 
    elif method == 'b':
        shape = [1, input_depth, spatial_size[0], spatial_size[1]]
        net_input = torch.zeros(shape)
        net_input=torch.Tensor(np.random.default_rng().beta(1,3,net_input.shape))
    elif method == 'g':
        shape = [1, input_depth, spatial_size[0], spatial_size[1]]
        net_input = torch.zeros(shape)
        net_input=torch.Tensor(np.random.default_rng().gumbel(size=net_input.shape))
    elif method=="n2":
        shape = [1, input_depth, spatial_size[0], spatial_size[1]]
        net_input = torch.zeros(shape)
        net_input=torch.Tensor(np.random.default_rng().normal(0,size=net_input.shape) +np.random.default_rng().normal(0.5,size=net_input.shape) )
