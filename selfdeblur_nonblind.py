...
parser.add_argument('--kernel_n', type=str)
...
for f in files_source:
    ...
    _, imgs = get_image(path_to_image, odd=True) # load image and convert to np.
    print(imgs.shape)
    y = np_to_torch(imgs).type(dtype)
    img_size = imgs.shape

    path_to_kernel = os.path.join(opt.save_path, "%s_k.png" % opt.kernel_n)
    print(path_to_kernel)
